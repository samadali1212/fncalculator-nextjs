 ...
 * @param location Job location (can be region, city, or specific location)
...
export const filterJobs = (
  ...
  return jobsData.filter(job => {
    ...
    if (location) {
      ...
      const citiesInRegion = getCitiesByRegion(location);
      if (citiesInRegion.length > 0) {
        // It's a region - check if job is in any city of this region or is nationwide/remote
        matchesLocation = job.location === 'Nationwide' || 
                         job.location === 'Remote' || 
                         citiesInRegion.includes(job.location);
      } else {
...
/**
 * Get all regions from job locations
 * @returns Array of unique regions
 */
export const getRegions = (): string[] => {
  // In Tanzania, the major regions are:
  const regions = [
    'Dar es Salaam',
    'Arusha',
    'Kilimanjaro',
...
/**
 * Map region to its major cities
...
export const getCitiesByRegion = (region: string): string[] => {
  const regionCityMap: Record<string, string[]> = {
    ...
    'Mwanza': ['Mwanza', 'Musoma', 'Bukoba'],
    'Mbeya': ['Mbeya', 'Kyela', 'Tukuyu'],
    'Morogoro': ['Morogoro', 'Kilosa', 'Mvomero'],
    'Tanga': ['Tanga', 'Pangani', 'Muheza'],
    'Dodoma': ['Dodoma', 'Kondoa', 'Mpwapwa'],
    'Kilimanjaro': ['Moshi', 'Same', 'Hai'],
...
export const getRegionByCity = (city: string): string => {
  ...
  return cityRegionMap[city] || 'Unknown';
};
...
export const getJobsByRegion = (region: string): Job[] => {
  ...
  return jobsData.filter(job => {
    // Include nationwide and remote jobs in all region listings
 ...
 * @param location Location name (region or city)
 * @returns SEO-friendly title
 */
export const createLocationPageTitle = (location: string): string => {
  return `Jobs in ${location}, Tanzania | Latest Vacancies`;
};
...
export const createLocationPageDescription = (location: string, jobCount: number): string => {
  return `Browse ${jobCount} job opportunities in ${location}, Tanzania. Find and apply for the latest vacancies in ${location} across various industries and career levels.`;
};
...
export const getApplicationInstructions = (method: ApplicationMethod): string => {
  switch(method.type) {
    ...
    case 'letter':
      return `Send your application letter to the address provided.`;
    case 'other':
      return method.value;
    default:
...
export const parseSalaryRange = (salaryRange: string): { 
  ...
  const result: {
    ...
    currency: "TZS", // Changed to Tanzanian Shilling
    unitText: "MONTH"
  };
  ...
  if (salaryRange.includes("$")) {
    ...
    result.currency = "GBP";
  } else if (salaryRange.includes("TSh") || salaryRange.includes("TZS")) {
    result.currency = "TZS";
  }
