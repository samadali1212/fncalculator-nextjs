...
export type SouthAfricaTimeFrame = "monthly" | "yearly";

export interface SouthAfricaTaxCalculationResult {
  grossIncome: number;
 ...
 */
export function calculateSouthAfricaIncomeTax(
  income: number,
  ageGroup: AgeGroup = "below65",
  timeFrame: SouthAfricaTimeFrame = "monthly"
 ...
 */
export function generateSouthAfricaTaxCalculations(
  min: number = 4000,
  max: number = 650000,
  step: number = 100,
  ageGroup: AgeGroup = "below65",
  ...
  const results: SouthAfricaTaxCalculationResult[] = [];
  
  for (let income = min; income <= max; income += step) {
    results.push(calculateSouthAfricaIncomeTax(income, ageGroup, timeFrame));
  }
 ...
 */
export function getSouthAfricaTaxCalculation(
  income: number,
  ageGroup: AgeGroup = "below65",
  timeFrame: SouthAfricaTimeFrame = "monthly"
): SouthAfricaTaxCalculationResult | null {
  if (isNaN(income) || income < 0) return null;
  return calculateSouthAfricaIncomeTax(income, ageGroup, timeFrame);
}
 ...
 */
export function formatSouthAfricaCurrency(value: number, fractionDigits: number = 0): string {
 ...
 */
export function convertSouthAfricaIncome(amount: number, fromTimeFrame: SouthAfricaTimeFrame): number {
