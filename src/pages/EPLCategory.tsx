...
import { 
  findEPLCategoryBySlug, 
  getEPLCategoryIdBySlug, 
  getPlayersByCategory,
} from "../utils/eplCategoriesData";
import { formatWeeklySalary, EPLPlayer } from "../utils/eplPlayersData";
...
const EPLCategory = () => {
  ...
  const category = slug ? findEPLCategoryBySlug(slug) : undefined;
  const categoryId = slug ? getEPLCategoryIdBySlug(slug) : undefined;
  
  useEffect(() => {
    if (categoryId) {
      // Simulate loading state for better UX
      const timer = setTimeout(() => {
        setPlayers(getPlayersByCategory(categoryId));
        setIsLoading(false);
      }, 500);
  ...
  if (!category) {
    return (
      <div className="min-h-screen bg-[#f6f6f0]">
        <Header />
        <main className="container mx-auto pt-24 px-4 pb-16 flex flex-col items-center justify-center">
          <h1 className="text-2xl font-bold mb-4">Category Not Found</h1>
          <p className="text-gray-600 mb-6">The category you're looking for doesn't exist.</p>
          <Button asChild>
            <Link to="/epl-categories">View All Categories</Link>
          </Button>
        </main>
      </div>
    );
  ...
  return (
    ...
    >
      <SEO 
        title={`${category.title} - EPL Player Salaries`}
        description={category.description}
        canonicalUrl={`/epl-players/category/${category.slug}`}
      />
      <Header />
      ...
      <main className="container mx-auto pt-24 px-4 md:px-6 pb-16 max-w-4xl">
        <div className="flex items-center justify-between mb-6">
          ...
          <ShareButton 
            title={`${category.title} - EPL Player Salaries - SalaryList`} 
            variant="outline"
          />
        </div>
        
        <div className="mb-8">
          <h1 className="text-2xl md:text-3xl font-bold mb-3">{category.title}</h1>
          <p className="text-gray-600 mb-4">
            {category.description}
          </p>
          {category.imageUrl && (
            <div className="aspect-w-16 aspect-h-9 rounded-md overflow-hidden shadow-sm mb-6">
              <img 
        ...
        >
          <div className="relative">
            ...
            <Input
              type="text"
              placeholder="Search players in this category..."
              className="pl-10"
              value={searchQuery}
