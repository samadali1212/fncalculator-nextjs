...
import { 
  ...
  formatNetWorth,
  formatAge,
  NetWorthPerson as ImportedNetWorthPerson
} from "../utils/globalNetWorthData";
import { createComparisonUrl } from "../utils/utils";

// Use the imported NetWorthPerson type
type NetWorthPerson = ImportedNetWorthPerson;

const GlobalCompareNetWorth = () => {
  ...
  // Helper function to calculate the age difference between two people
  const getAgeDifference = (person1: NetWorthPerson, person2: NetWorthPerson): number => {
    const birthDate1 = new Date(person1.dateOfBirth);
    const birthDate2 = new Date(person2.dateOfBirth);
    
    // Calculate difference in years
    const yearDiff = Math.abs(birthDate1.getFullYear() - birthDate2.getFullYear());
    
    // Adjust for months and days if needed for more accuracy
    const monthDiff = birthDate1.getMonth() - birthDate2.getMonth();
    if (monthDiff > 0 && birthDate1.getFullYear() > birthDate2.getFullYear()) {
      return yearDiff - 1;
    } else if (monthDiff < 0 && birthDate1.getFullYear() < birthDate2.getFullYear()) {
      return yearDiff - 1;
    } else if (monthDiff === 0) {
      const dayDiff = birthDate1.getDate() - birthDate2.getDate();
      if ((dayDiff > 0 && birthDate1.getFullYear() > birthDate2.getFullYear()) || 
          (dayDiff < 0 && birthDate1.getFullYear() < birthDate2.getFullYear())) {
        return yearDiff - 1;
      }
    }
  ...
  // Helper function to determine who is older
  const getOlderPerson = (person1: NetWorthPerson, person2: NetWorthPerson): NetWorthPerson => {
    const birthDate1 = new Date(person1.dateOfBirth);
    const birthDate2 = new Date(person2.dateOfBirth);
    
    return birthDate1 < birthDate2 ? person1 : person2;
  };
  ...
  const generateComparisonText = () => {
    ...
    if (richer?.industry === poorer?.industry) {
      ...
      text += `While ${richer?.name} made a fortune in the ${richer?.industry} industry, ${poorer?.name} accumulated wealth in the ${poorer?.industry} sector. `;
    }
    ...
    // Calculate age difference using the new helper function
    if (richer && poorer) {
      const ageDiff = getAgeDifference(richer, poorer);
      if (ageDiff > 0) {
        const older = getOlderPerson(richer, poorer);
        text += `There is a ${ageDiff} year age difference between them, with ${older.name} being older. `;
      }
    }
                  ...
                  <Table>
                    ...
                    <TableBody>
                      ...
                      <TableRow>
                        ...
                        <TableCell>{formatAge(person1.dateOfBirth)}</TableCell>
                        <TableCell>{formatAge(person2.dateOfBirth)}</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                </div>
