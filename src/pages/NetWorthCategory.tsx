...
const NetWorthCategory = () => {
  ...
  // Filter people based on search query and category
  const filteredPeople = netWorthPeople.filter(person => {
    ...
    const matchesCategory = !category || person.categories.includes(category);
  ...
  return (
    ...
    >
      <SEO 
        title={`${categoryTitle} Net Worth | SalaryList`} 
        description={`Explore the net worth of ${categoryTitle.toLowerCase()} in South Africa. Updated list with detailed wealth information.`}
        canonicalUrl={`/net-worth/category/${category || ''}`}
      />
      <Header />
      ...
      <main className="container mx-auto pt-24 px-4 md:px-6 pb-16 max-w-4xl">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold mb-2">{categoryTitle} Net Worth</h1>
            <p className="text-gray-600">
              Explore the net worth of {categoryTitle.toLowerCase()} in South Africa
            </p>
          </div>
        </div>
